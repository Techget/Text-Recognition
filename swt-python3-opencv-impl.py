...
class SWTScrubber(object):
    ...
    def _create_derivative(cls, filepath):
        ...
        if diagnostics:
            global filename_without_suffix
            cv2.imwrite('swt/'+filename_without_suffix+'/edges.jpg',edges)
            cv2.imwrite('swt/'+filename_without_suffix+'/sobelx64f.jpg', np.absolute(sobelx64f))
            cv2.imwrite('swt/'+filename_without_suffix+'/sobely64f.jpg', np.absolute(sobely64f))
            # amplify theta for visual inspection
            theta_visible = (theta + np.pi)*255/(2*np.pi)
            cv2.imwrite('swt/'+filename_without_suffix+'/theta.jpg', theta_visible)
    ...
    def _swt(self, theta, edges, sobelx64f, sobely64f):
        ...
        if diagnostics:
            global filename_without_suffix
            cv2.imwrite('swt/'+filename_without_suffix+'/swt.jpg', swt * 100)
    ...
    def _find_letters(cls, swt, shapes):
        ...
        for label,layer in shapes.items():
            ...
            if diagnostics:
                ...
                global filename_without_suffix
                cv2.imwrite('swt/'+filename_without_suffix+'/layer'+ str(label) +'.jpg', layer * 255)
...
filename_without_suffix = os.path.splitext(os.path.basename(filename))[0]

if not os.path.exists('swt/'):
    os.mkdir('swt/')

if not os.path.exists('swt/'+filename_without_suffix+'/'):
    os.mkdir('swt/'+filename_without_suffix+'/')

final_mask = SWTScrubber.scrub(filename)
cv2.imwrite('swt/'+filename_without_suffix+'/final.jpg', final_mask * 255)
